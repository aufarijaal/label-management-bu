...
export default function InputTypesPage() {
    ...
    const [rows, setRows] = React.useState<InputType[]>([]);
    ...
    const [deleteTarget, setDeleteTarget] = React.useState<InputType | null>(null);
    ...
    const openEditDialog = (row: InputType) => {
        setDialogMode('edit');
        setFormId(row.id);
    ...
    const handleSave = async () => {
        ...
        setSaving(false);
        setDialogOpen(false);
        setSnackbar({ open: true, message: dialogMode === 'add' ? 'Input type created.' : 'Input type updated.', severity: 'success' });
        fetchRows();
    };
    ...
    const handleDeleteConfirm = async () => {
        ...
        if (error) {
            const message = error.code === '23503'
                ? 'Cannot delete: this input type is referenced by existing avery note items.'
                : error.message;
            setSnackbar({ open: true, message, severity: 'error' });
        } else {
            setSnackbar({ open: true, message: 'Input type deleted.', severity: 'success' });
            fetchRows();
        }
    ...
    return (
        <AppLayout>
            <Head>
                <title>Input Types — LabelManager</title>
                <meta name="viewport" content="width=device-width, initial-scale=1" />
            </Head>

            <Box sx={{ py: 4, px: { xs: 2, md: 4 }, flex: 1 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 4 }}>
                    <InputIcon sx={{ color: 'primary.main', fontSize: 32 }} />
                    <Box>
                        <Typography variant="h4" fontWeight={700}>
                            Input Types
                        </Typography>
                        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                            Manage the list of input types
                        </Typography>
                    </Box>
                    <Box sx={{ ml: 'auto' }}>
                        <Chip
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={3} align="center" sx={{ py: 6 }}>
                                                <Typography variant="body2" color="text.secondary">
                                                    {search ? 'No records match your search.' : 'No input types found.'}
                                                </Typography>
                                            </TableCell>
                                        </TableRow>
                                    ) : (
            ...
            <Dialog open={dialogOpen} onClose={closeDialog} fullWidth maxWidth="xs">
                <DialogTitle>{dialogMode === 'add' ? 'Add Input Type' : 'Edit Input Type'}</DialogTitle>
                <DialogContent>
            ...
            <Dialog open={!!deleteTarget} onClose={() => !deleting && setDeleteTarget(null)} fullWidth maxWidth="xs">
                <DialogTitle>Delete Input Type</DialogTitle>
                <DialogContent>
                    <DialogContentText>
